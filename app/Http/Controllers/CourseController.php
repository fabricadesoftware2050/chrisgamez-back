...
foreach ($data as $curso) {
    ...
    // Asignar datos al curso
    $modelo->progreso = $cursoMinutosTotales > 0
        ? round(($cursoMinutosVistos / $cursoMinutosTotales) * 100)
        : 0;
...
// Agregar datos globales a la respuesta
$modelo->total_horas_vistas = round($totalMinutosVistos / 60, 2);
$modelo->total_horas_totales = round($totalMinutosDisponibles / 60, 2);
$modelo->total_progreso_global = $totalMinutosDisponibles > 0
    ? round(($totalMinutosVistos / $totalMinutosDisponibles) * 100)
    : 0;
